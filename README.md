...
And it refuses to fabricate a number to get there: attribution is genuinely ambiguous, so two counts are always shown and never averaged; context no file explains renders as *claude code overhead* at full size, with a footnote saying what's in it; thin evidence is badged, not hidden. Zero dependencies, read-only, one process — nothing leaves your machine.

## What it shows

- **Consumption** — token flow as small multiples: *context re-read* (cache) and *fresh work* (input/output) as separate panels at their own honest scales, because cache is ~99% of the stack and a shared scale renders new work invisible. Drill from day to hour to the event level; an approximate cost at public list price that states its coverage (models without a public price are excluded, never guessed — you can supply prices in `~/.ccwhere/overrides.json`); a by-model breakdown that follows the drill; and a league table of consumers ranked *two ways* — by whole session tokens and by direct message tokens — shown side by side and never averaged. Agreement between the two counts is the signal; disagreement is flagged, not hidden. Consumer-type chips scope the league; sparklines click-enlarge to dated charts with a uses/tokens mode; an experimental toggle re-counts both columns over fresh work only.
- **Context ledger** — what every session pays before any work happens, as a folder tree from your user scope down each observed session path: tokens *added here* per level and the *total so far* along the branch, beside the measured first-call median (the authoritative number) and the *claude code overhead* no file predicts — system prompt, built-in tools, MCP schemas, hook output — never normalized away, itemized in a footnote. Costs are probe-calibrated and three-tier honest: skills and plugins cost a small fixed amount up front (~25 tok/entry, measured), their descriptions load on use, and their full text loads when run — all three shown. Folders whose skills exist on disk but have never hosted a session appear as marked (†) rows with no invented numbers — present, but nothing loaded yet, so nothing to measure. Drills to a per-skill pruning list: description and full-text size, usage history, first/last used.
...
## Why token attribution is ambiguous — and why two numbers are shown
...
- **Whole session tokens** — the total tokens of every session the consumer appeared in. An upper bound; reads high, because one long session inflates everything it touched.
- **Direct message tokens** — only the tokens of the exact messages that used it. A lower bound; reads low, because a consumer's real cost lands after the call.

The signal is **agreement**: a consumer ranked top-5 on both counts is genuinely expensive, and gets flagged as such. Disagreement — high on one count, low on the other — is flagged too, and usually means one long session inflating it rather than a real burner: inspect before acting. Averaging the two would manufacture precision that does not exist, and is banned in the spec.
...
The first three rank by default because they're actionable. Shell utilities and built-ins sit behind toggles for two reasons: you can't prune `grep`, and because they appear in nearly every session, whole-session counting hands them everything and they drown the ranking.
