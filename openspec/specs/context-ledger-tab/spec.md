...
The ledger SHALL render a folder tree from the user scope down each observed session path (content-less intermediate folders skipped): per node, "additional" eager tokens introduced at that level and "accumulated" — the parent's accumulated plus additional, along one branch. Nodes with sessions SHALL show the measured median and the unattributed share (median − accumulated, clamped at zero; displayed in plain language as "claude code overhead", with a footnote itemizing what falls into it) beside the estimate, with low-sample badges under N=3. Each node SHALL expand to its items (instruction files, stubs with catalogs, project-enabled plugins) and onward into the per-skill usage drill. The user-level node SHALL acknowledge installed-but-not-user-enabled plugins with an informational row (no invented tokens) pointing to the project nodes that cost them — the full installed inventory must be discoverable from the top of the tree. The UI SHALL state the single-branch assumption — visiting a sibling folder mid-session loads its content on use, not modeled — and the stub calibration with its measured range. Content-bearing subfolders of an observed session root that were never themselves a session root SHALL appear as dormant nodes: marked with a footnoted indicator and carrying no number in additional, accumulated, or median — present but never loaded, so no measured token load exists — while their inventory remains drillable with file-fact sizes showing what a session opened there would add. A dormant folder whose skill/command scaffold exists but is empty SHALL still appear, carrying an informational row that nothing can load from it; a bare `.claude` with no scaffold stays invisible.

#### Scenario: Dormant folder is visible but never costed
- **WHEN** a subfolder holds `.claude/skills` but no session was ever opened at it
- **THEN** it appears as a marked node with dashes in all token columns, a footnote explaining nothing has loaded there yet, and a drill listing its skills
